...
void CtxItemActionCnt::extract(Sample *sample, Map<string, Feature> *features) {
    ...
    if (features->contains(CtxItem::ctx_item_stat)) {
        for (auto x: features->find(CtxItem::ctx_item_stat)->second.item_action_list().value())
            list.emplace_back(x);
    }
    vector<ItemAction> list_2week = filter(list, 14, now_time);
    vector<ItemAction> list_1week = filter(list, 7, now_time);
    vector<ItemAction> list_3day = filter(list, 3, now_time);
    vector<ItemAction> list_1day = filter(list, 1, now_time);
    //event,ctx,search
...
vector<ItemAction> CtxItemActionCnt::filter(vector<ItemAction> list, int days, long now_time) {
    if(list.empty())return list;
    std::remove_if(list.begin(),list.end(),[this, days, now_time](ItemAction x) {
        return diffDay(now_time, date2Unix(x.date())) > days;
    });
    return list;
}

Map<string, Five<int, int, int, int, int>> CtxItemActionCnt::statPv(vector<ItemAction> list, string flag) {
...
void CtxItemActionCnt::extract(Sample *sample, vector<ItemAction> list, string flag, string prefix) {
...
//    stat_list = padding(stat_list, flag); //对于item特征，都需要padding
    sample->addLongListWeight("ctx_stat_" + flag + "_" + prefix, stat_list);
    ...
    addFloatList(sample, "ctx_cxr_" + prefix, ctx);
}


Five<float, float, float, float, float>
CtxItemActionCnt::cxr(Five<int, int, int, int, int> clk, Five<int, int, int, int, int> exp) {
...
vector<pair<long, float>> CtxItemActionCnt::padding(vector<pair<long, float>> stat_list, string flag) {
